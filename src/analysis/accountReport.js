...
function deriveSignals(report) {
  ...
  if (audience.followerRatio >= 2) {
    signals.push({
      ...
      detail: `${formatCount(audience.followers)} followers against ${formatCount(audience.following)} following is a ratio of ${formatCount(audience.followerRatio)}, which reads as an account people seek out.`,
    });
  } else if (audience.following > 1000 && audience.followerRatio < 1) {
    signals.push({
      level: 'watch',
      title: 'Follows more accounts than follow back',
      detail: `A ratio of ${formatCount(audience.followerRatio)} across ${formatCount(audience.following)} follows is the pattern of follow-for-follow growth rather than pull.`,
    });
  }
  ...
  } else if (engagement.medianPerOriginal > 0 && engagement.rate < 0.1 && audience.followers > 10_000) {
    // Guarding on medianPerOriginal rather than on the rate matters: at a few
    // million followers a real rate can round to 0.000 at three decimals, and
    // guarding on `rate > 0` silently dropped exactly the accounts this
    // observation exists to flag.
    signals.push({
      level: 'watch',
      title: engagement.rate > 0 ? `Engagement rate of ${engagement.rate}%` : 'Engagement rate below 0.001%',
      detail: `A median original post draws ${formatCount(engagement.medianPerOriginal)} interactions against ${formatCount(audience.followers)} followers. Below 0.1% at this follower count usually means reach is being throttled, the audience is stale, or the follower number is inflated.`,
    });
  }
