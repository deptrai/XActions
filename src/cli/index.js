...
program
  ...
  // --- messenger-share inputs (Story 5.4) — inline flag beats file ---
  ...
  .action(async (options) => {
    ...
    // --- messenger-share: resolve inline-or-file inputs + build queue (Story 5.4) ---
    // Inline flags take precedence over files (log which was used). All reads happen
    // BEFORE browser launch so validation fails fast on bad inputs.
    let campaigns = [];
    if (action === 'messenger-share') {
      ...
      // recipients: inline --recipients (comma) beats --recipients-file
      let recipients = [];
      ...
      // content: inline --content beats --content-file
      let content = '';
      ...
      // post URL(s): inline --post-url beats --links-file
      let linksText = '';
      if (options.postUrl) {
      ...
      // Fail-fast validation (mirror existing guards) BEFORE browser launch.
      if (!stats.links) {
    ...
    try {
      ...
      if (action === 'like') {
        ...
        // One campaign per link (per buildCampaignQueue); per-recipient batching
        // stays inside runGuardedBatch — no nested custom loop here.
        const runs = [];
