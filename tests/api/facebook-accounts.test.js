...
describe('encrypt / decrypt roundtrip', () => {
  it(`[${nextTestId('API', 'P2')}] decrypt(encrypt(x)) === x for a cookie JSON payload`, () => {
    const payload = JSON.stringify({ c_user: '1234567890', xs: 'xs_token_abc' });
    expect(decrypt(encrypt(payload))).toBe(payload);
  });

  it(`[${nextTestId('API', 'P2')}] decrypt returns null for invalid/garbage input (no throw)`, () => {
    expect(decrypt('not-valid')).toBeNull();
    expect(decrypt('')).toBeNull();
    expect(decrypt(null)).toBeNull();
  });

  it(`[${nextTestId('API', 'P2')}] encrypted value never equals the plaintext`, () => {
  ...
  it(`[${nextTestId('API', 'P2')}] two encryptions of same plaintext produce different ciphertext (random IV)`, () => {
...
describe('NFR3 — encrypt never leaks cookie values in output', () => {
  it(`[${nextTestId('API', 'P2')}] encrypted output does not contain raw c_user value`, () => {
    const c_user = '9876543210';
    const enc = encrypt(JSON.stringify({ c_user, xs: 'xs_val' }));
    expect(enc).not.toContain(c_user);
  });

  it(`[${nextTestId('API', 'P2')}] encrypted output does not contain raw xs value`, () => {
...
describe('encrypt / decrypt — exact encoding (P1 kill)', () => {
  it(`[${nextTestId('API', 'P2')}] encrypt output is hex-encoded (not empty string replacement)`, () => {
  ...
  it(`[${nextTestId('API', 'P2')}] decrypt returns exact plaintext for roundtrip (encoding strings matter)`, () => {
  ...
  it(`[${nextTestId('API', 'P2')}] decrypt returns null for 3-part string (not 4 parts)`, () => {
  ...
  it(`[${nextTestId('API', 'P2')}] decrypt returns null for 5-part string (not 4 parts)`, () => {
    expect(decrypt('a:b:c:d:e')).toBeNull();
  });

  it(`[${nextTestId('API', 'P2')}] decrypt handles tampered ciphertext (auth tag mismatch → null)`, () => {
  ...
  it(`[${nextTestId('API', 'P2')}] encrypt output parts are valid hex strings (encoding strings matter)`, () => {
  ...
  it(`[${nextTestId('API', 'P2')}] decrypt with wrong encoding returns null or wrong value (encoding strings matter)`, () => {
  ...
  it(`[${nextTestId('API', 'P2')}] decrypt with 3 parts returns null (parts.length check)`, () => {
  ...
  it(`[${nextTestId('API', 'P2')}] decrypt with 5 parts returns null (parts.length check)`, () => {
    // 5 parts should also be rejected
    const result = decrypt('aabb:bccd:eeff:0011:2233');
    expect(result).toBeNull();
  });

  it(`[${nextTestId('API', 'P2')}] decrypt with exactly 4 valid hex parts but wrong data returns null`, () => {
    // 4 parts but not valid encrypted data → should return null (not throw)
    expect(decrypt('aaaa:bbbb:cccc:dddd')).toBeNull();
  });

  it(`[${nextTestId('API', 'P2')}] c_user with leading/trailing spaces is trimmed before regex test`, () => {
  ...
  it(`[${nextTestId('API', 'P2')}] c_user with only leading spaces is trimmed`, () => {
...
describe('validateAccountBody — boundary + type (P1 kill)', () => {
  it(`[${nextTestId('API', 'P2')}] accepts label of exactly 50 chars (boundary, not >)`, () => {
  ...
  it(`[${nextTestId('API', 'P2')}] rejects label of 51 chars (boundary)`, () => {
    expect(validateAccountBody({ label: 'a'.repeat(51), c_user: '1234567890', xs: 'xs' }))
      .toMatch(/50/);
  });

  it(`[${nextTestId('API', 'P2')}] rejects non-string label (number)`, () => {
  ...
  it(`[${nextTestId('API', 'P2')}] rejects non-string label (object)`, () => {
    expect(validateAccountBody({ label: { foo: 'bar' }, c_user: '1234567890', xs: 'xs' }))
      .toMatch(/label/i);
  });

  it(`[${nextTestId('API', 'P2')}] trim is applied before length check on label`, () => {
  ...
  it(`[${nextTestId('API', 'P2')}] c_user is coerced to string before regex test`, () => {
  ...
  it(`[${nextTestId('API', 'P2')}] rejects non-string xs (number)`, () => {
  ...
  it(`[${nextTestId('API', 'P2')}] rejects non-string xs (object)`, () => {
    expect(validateAccountBody({ label: 'x', c_user: '1234567890', xs: { foo: 'bar' } }))
      .toMatch(/xs/i);
  });

  it(`[${nextTestId('API', 'P2')}] accepts xs of exactly 4096 chars (boundary, not >)`, () => {
  ...
  it(`[${nextTestId('API', 'P2')}] rejects xs of 4097 chars (boundary)`, () => {
    expect(validateAccountBody({ label: 'x', c_user: '1234567890', xs: 'a'.repeat(4097) }))
      .toMatch(/4096/);
  });

  it(`[${nextTestId('API', 'P2')}] trim is applied before length check on xs`, () => {
  ...
  it(`[${nextTestId('API', 'P2')}] rejects whitespace-only xs`, () => {
