...
const TEST_SCOPE = 'e2e-api-operations';
...
describe('Operations endpoints', () => {
  ...
  it(`[${nextTestId(TEST_SCOPE, 'E2E', 'P2')}] GET /api/operations without auth → 401`, async () => {
    const res = await request(app).get('/api/operations');
    expect(res.status).toBe(401);
    expect(res.body).toHaveProperty('error');
  });

  it(`[${nextTestId(TEST_SCOPE, 'E2E', 'P2')}] GET /api/operations with malformed token → 401`, async () => {
  ...
  it(`[${nextTestId(TEST_SCOPE, 'E2E', 'P2')}] GET /api/operations with no Bearer scheme → 401`, async () => {
    const res = await request(app)
      .get('/api/operations')
  ...
  it(`[${nextTestId(TEST_SCOPE, 'E2E', 'P2')}] POST /api/operations/cancel/:id without auth → 401`, async () => {
    const res = await request(app).post(`/api/operations/cancel/${makeOperationId()}`);
    expect(res.status).toBe(401);
    expect(res.body).toHaveProperty('error');
  });

  it(`[${nextTestId(TEST_SCOPE, 'E2E', 'P2')}] POST /api/operations/unfollow-non-followers without auth → 401`, async () => {
    const res = await request(app)
  ...
  it(`[${nextTestId(TEST_SCOPE, 'E2E', 'P2')}] GET /api/operations with fake (non-existent user) JWT → 401`, async () => {
  ...
  it.each([
    [nextTestId(TEST_SCOPE, 'E2E', 'P1'), 'unfollow-non-followers', { maxUnfollows: 10 }],
    [nextTestId(TEST_SCOPE, 'E2E', 'P1'), 'unfollow-everyone', { maxUnfollows: 10 }],
    [nextTestId(TEST_SCOPE, 'E2E', 'P1'), 'detect-unfollowers', {}],
  ...
  it(`[${nextTestId(TEST_SCOPE, 'E2E', 'P2')}] GET /api/operations?page=1&limit=5 without auth → 401`, async () => {
    const res = await request(app).get('/api/operations?page=1&limit=5');
    expect(res.status).toBe(401);
  });

  it(`[${nextTestId(TEST_SCOPE, 'E2E', 'P2')}] GET /api/operations?status=pending without auth → 401`, async () => {
