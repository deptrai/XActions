...
describe('Story 12.1 — QR Code Utility (src/utils/qrcode.js)', () => {
  ...
  describe('displayTerminalQrCode(data, options)', () => {
    it('[P0] should render ASCII QR matrix on TTY terminal with 1:1 ratio blocks', async () => {
      ...
      // Should contain unicode block characters or spaces used in ASCII QR rendering
      expect(output).toMatch(/[\u2588\u2580\u2584\s]/);
    });

    it('[P0] should automatically use small matrix when terminal width is narrow (< 80 columns)', async () => {
      process.stdout.isTTY = true;
    ...
    it('[P0] should render plain URL and short code on Non-TTY environments without terminal escapes', async () => {
    ...
    it('[P1] should include plain text URL below QR when options.showUrl is true', async () => {
      process.stdout.isTTY = true;
    ...
    it('[P1] should throw PlatformError or formatted error if data is empty or invalid', async () => {
  ...
  describe('isTty() helper', () => {
    it('[P0] should accurately reflect process.stdout.isTTY state', () => {
      process.stdout.isTTY = true;
