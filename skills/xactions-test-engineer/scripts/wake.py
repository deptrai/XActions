...
def main() -> int:
    ...
    parser.add_argument(
        "--pulse",
        action="store_true",
        help="Pulse mode: also append PULSE.md for autonomous waking.",
    )
    ...
    print("MODE: PULSE" if args.pulse else "MODE: WAKING")
    print(f"Sanctum: {sanctum}")
    print(json.dumps({"mode": "PULSE" if args.pulse else "WAKING", "sanctum": str(sanctum)}))
