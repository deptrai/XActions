...
| AD-7.2 | Thêm `proxy` field vào `FacebookAccount` | Mỗi account cần gắn proxy cố định để tránh IP leak hoặc ghép nhầm proxy khi chạy song song. | `FacebookAccountPool` phải honor proxy affinity; `proxy` lưu flat string `host:port[:user:pass]`, parse qua `parseFlatProxy`; cần migration thêm `proxy` (nullable string). |
