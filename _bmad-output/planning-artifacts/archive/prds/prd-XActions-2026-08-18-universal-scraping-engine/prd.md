...
* **FR-73A (AI Streaming Dataset Exporter):** Xuất dữ liệu từ PostgreSQL ra định dạng JSON Lines (`.jsonl`) và CSV dạng stream có xử lý Backpressure (`drain` event), sanitize `\r\n` và hỗ trợ nén Gzip.

### Nhóm 2: Mạng & Quản Lý Proxy Bọc Thép (Epic 11)
* **FR-66A (Anti-Leak Proxy Pool):** Quản lý tập trung Static & Dynamic Tunnel Proxy, tự động kích hoạt cờ chống rò rỉ WebRTC/DNS (`--force-webrtc-ip-handling-policy=disable_non_proxied_udp`) và kiểm tra buffer expiration 30s.
* **FR-66B (429/403 Auto-Quarantine & Standby Backoff):** Cách ly proxy lỗi 5 phút khi gặp mã `429/403`, tự động đổi IP và retry 3 lần với exponential backoff. Chuyển sang Standby Backoff 30s khi 100% proxy bị chặn.
...
* **Phase 1: Foundation & Resilient Infrastructure (Stories 10.1 ➔ 10.3, 11.1 ➔ 11.3, 12.1 ➔ 12.2)** *(8 stories)*
