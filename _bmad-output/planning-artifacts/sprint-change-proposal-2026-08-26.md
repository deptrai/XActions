...
| **13.2** | ready-for-dev | Cần nhưng **phải scope chặt** | **Cao** — `src/client/Scraper.js` và `src/scrapers/twitter/http/index.js` đã có profile/tweets/followers/search/DMs/media. | 13.2 chỉ làm search/timeline; các tính năng còn lại được lên kế hoạch trong Story 13.2.1–13.2.9. |
...
**Story 13.2 (Twitter Hybrid):** Chỉ làm `search(query)` và `getTimeline(username)`. Profile/followers/following, thread/likes/bookmarks, search/hashtag/trending, media, lists/communities/spaces, content composition/schedule, engagement/social-graph, DM/lists, và integration/caller migration được tách ra thành Story 13.2.1–13.2.9 trong Epic 13.
