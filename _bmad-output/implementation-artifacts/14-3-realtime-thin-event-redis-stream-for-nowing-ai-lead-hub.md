...
<!-- Generated by bmad-code-review on 2026-08-28 -->

#### decision-needed

- [ ] [Review][Decision] **ThreadsCrawler chưa được cập nhật để dùng RedisStreamPublisher** — `src/scrapers/social/threads/crawler.js` không có trong diff của commit 14.3. Spec Task T5 / AC-5 yêu cầu chuyển từ raw `xadd` sang `RedisStreamPublisher`, dùng `isEnvTruthy(REDIS_STREAM_ENABLED)`, và áp dụng `MAXLEN`/`MINID` trimming. FacebookCrawler đã tích hợp đầy đủ. Cần quyết định: bổ sung trong story 14.3 hay tách thành story follow-up?

#### patch

- [ ] [Review][Patch] **`toIsoDate` throw `RangeError` trên `Invalid Date` instance** `src/utils/redis-stream-publisher.js:27-35` — Nhánh `if (val instanceof Date) return val.toISOString();` nằm ngoài `try/catch`; `new Date('invalid') instanceof Date === true` và `toISOString()` ném `RangeError`.
- [ ] [Review][Patch] **`PrismaStore.saveCheckpoint` không kiểm tra ngày hợp lệ** `src/store/prisma-store.js:350-352` — `new Date(checkpoint.lastTimestamp)` / `new Date(checkpoint.lastCrawledAt)` tạo `Invalid Date` khi đầu vào không hợp lệ, gây lỗi Prisma.
- [ ] [Review][Patch] **`/admin/stream/metrics` và `/admin/stream/alerts` thiếu auth** `api/server.js:390-406` — Các endpoint được mount trực tiếp ở root thay vì dưới `/api/admin` và không dùng `authenticateToken` / `requireAdmin`.
- [ ] [Review][Patch] **`xgroupEnsure` dùng ID `'$'` làm mất batch dữ liệu đầu tiên** `src/utils/redis-stream-publisher.js:331, 335, 339` — `XGROUP CREATE ... $ MKSTREAM` khiến consumer group chỉ nhận message sau thời điểm tạo; nên dùng `'0'` để nhận toàn bộ message hiện có.
- [ ] [Review][Patch] **Auto-sync consumer lag sang `AdaptiveRateGovernor` bị thiếu** `src/utils/stream-metrics-collector.js`, `src/core/adaptive-governor.js` — `updateRedisConsumerLag(lag)` tồn tại nhưng không được gọi tự động; cần interval 15-30s hoặc gọi từ metrics endpoint.
- [ ] [Review][Patch] **`XINFO CONSUMERS` fallback hardcode `lastAckTime = 0`** `src/utils/stream-metrics-collector.js:247-251` — Khi dùng `sendCommand`, code gán cứng `lastAckTime = 0` thay vì parse `idle`.
- [ ] [Review][Patch] **Webhook alert không có timeout** `src/utils/stream-alerts.js:154-158` — `fetch(webhookUrl, ...)` không dùng `AbortSignal.timeout()`, có thể treo vô thời hạn.
- [ ] [Review][Patch] **`/metrics/stream` gọi alert engine fire-and-forget** `api/server.js:378-388` — `defaultStreamAlertEngine.checkAndAlert(metrics)` được gọi không `await`; nên `await` hoặc dùng concurrency guard.
- [ ] [Review][Patch] **CLI port mismatch giữa `metrics` và `alerts`** `src/cli/commands/admin.js:31, 76` — `metrics` default `http://localhost:3001`, `alerts` default `http://localhost:3000`; cần thống nhất.
- [ ] [Review][Patch] **Tạo mới Nodemailer transport mỗi email alert** `src/utils/stream-alerts.js:172-183` — Nên cache transporter ở cấp class instance.
- [ ] [Review][Patch] **Log flooding trong `AdaptiveRateGovernor.getMaxThroughput`** `src/core/adaptive-governor.js:198-206` — `console.warn` gọi mỗi lần tính toán rate khi backpressure active; nên chỉ log khi chuyển trạng thái.

#### defer

- [x] [Review][Defer] **Race condition checkpoint trước stream publish** `src/scrapers/social/facebook/crawler.js:859-896` — Checkpoint được lưu trước khi lặp `publisher.publish()`; process crash giữa chừng có thể mất một số message. Đây là trade-off chấp nhận được theo AC-2; giảm thiểu hoàn toàn cần transaction hoặc batch publish phức tạp hơn, ngoài scope hiện tại.
