...
### Code Review Workflow Findings

The following findings were produced by the `bmad-code-review` workflow (Blind Hunter, Edge Case Hunter, Acceptance Auditor) against commit `df90d50` and the story spec.

#### [Review][Patch] findings (to fix)

- [x] [Review][Patch] IPRoyal geo/session parameters are appended to `username` instead of `password` as required by the provider preset matrix [`src/proxy/providers.js:494-499`]
- [x] [Review][Patch] Kuaidaili tunnel formatting is missing Normal/Pro modes; only appends `_session-<sid>` to username instead of `:<sid>` to password (Normal) or `user-period-<min>-sid-<sid>` (Pro) [`src/proxy/providers.js:502-506`]
- [x] [Review][Patch] Session IDs generated by `DynamicTunnelProvider.getProxy` contain underscores and exceed provider length/charset limits, and do not honor IPRoyal 8-char or Kuaidaili ≤6-char rules [`src/proxy/providers.js:528-538`]
- [x] [Review][Patch] `DynamicTunnelProvider.healthyCount` and `isAllQuarantined` are hardcoded to `1`/`false` and do not reflect quarantine state; missing `standbyBackoffMs` and `proxy_exhausted` when all sessions quarantined [`src/proxy/providers.js:414-426`]
- [x] [Review][Patch] `quarantine(proxy)` does not resolve the associated `accountId` from the session tag to invalidate only that account; it rotates every account seed [`src/proxy/providers.js:462-474`]
- [x] [Review][Patch] `DynamicTunnelProvider.getBrowserArgs` uses double-quoted and comma-spaced `--host-resolver-rules` and does not bracket IPv6, diverging from `ProxyIpPool.getBrowserArgs` [`src/proxy/providers.js:576-592`]
- [x] [Review][Patch] Provider auto-detection uses `includes` checks that can false-positive for unrelated hosts containing provider substrings [`src/proxy/providers.js:397-412`]
- [x] [Review][Patch] `DynamicTunnelProvider` accepts invalid `provider` preset values and silently falls back to returning raw username, instead of validating preset [`src/proxy/providers.js:389-390`, `src/proxy/providers.js:476-518`]
- [x] [Review][Patch] `quarantine` does not validate `durationMs` is positive and does not prune expired sessions at call time, allowing 0/negative durations and unbounded map growth [`src/proxy/providers.js:462-474`]
- [x] [Review][Patch] `createProxyProvider` error message stringifies full `config`, which may include `gatewayUrl` credentials and leak them to logs [`src/proxy/providers.js:638`]
- [x] [Review][Patch] `AbstractApiClient.resolveProxy` returns `null` when no provider is configured instead of throwing `proxy_exhausted` as required by AC-9 [`src/core/base-client.js:58-59`]
- [x] [Review][Patch] `AbstractApiClient.resolveProxy` does not validate the provider implements the expected contract; missing methods cause `TypeError` at runtime [`src/core/base-client.js:62-70`]
- [x] [Review][Patch] `DynamicTunnelProvider` does not support additional provider-specific parameters listed in the preset matrix (`sessionduration`, `state`, `region`, `isp`, `lifetime`, `period`, `sid`, `const`) [`src/proxy/providers.js:476-518`]
- [x] [Review][Patch] The custom preset default template is used even when no template is supplied; spec requires `custom` without an explicit template to throw `XACT_4001` [`src/proxy/providers.js:390`, `src/proxy/providers.js:508-515`]
- [x] [Review][Patch] `getProxy` does not validate `sessionDurationMs` is positive; negative values are accepted and break time-bucket math [`src/proxy/providers.js:392`]
- [x] [Review][Patch] `tests/proxy/providers-tunnel.test.js` asserts wrong IPRoyal and Kuaidaili formats and lacks coverage for exhausted provider, session ID limits, IPv6, and invalid provider preset [`tests/proxy/providers-tunnel.test.js:287-335`, `tests/proxy/providers-tunnel.test.js:398-419`]

#### [Review][Defer] findings (pre-existing / out of scope)

- [x] [Review][Defer] Session time-bucket and quarantine expirations depend on `Date.now()` and are sensitive to clock skew; monotonic clock out of scope (same as Story 11.1) [`src/proxy/providers.js:429`, `src/proxy/providers.js:529`] — deferred, pre-existing
- [x] [Review][Defer] No checkout/checkin between dynamic proxy session selection and the actual HTTP request; request-pipeline transaction belongs to Story 11.5/11.7 [`src/proxy/providers.js:520-551`] — deferred, pre-existing
...
- **Adversarial Code Review completed:** 12/12 review patches resolved (prior round), **Round 2 code review workflow** identified 16 patch findings and 2 deferred items; all 16 [Review][Patch] items applied and targeted/full regression suites verified.
- **Next phase:** Story 11.2 complete; move to Story 11.3 (`11-3-429-403-auto-quarantine-exponential-backoff-replay-interceptor.md`) or next priority.
