...
- [x] [Review][Patch][HIGH] FIXED — `JSHandle.click()` TypeError. Text fallback now does `const handle = await page.evaluateHandle(...); shareNowEl = handle.asElement();` → an `ElementHandle` (or null), so `.click()` is valid. [api/services/facebookAutomation.js#shareSinglePost]
- [x] [Review][Patch][HIGH] FIXED — over-matching share-button fallbacks. Removed `[aria-label*="Share"]` / `[aria-label*="Chia sẻ"]`; only the precise verified `data-ad-rendering-role` selectors remain. [api/services/facebookAutomation.js#shareSinglePost]
- [~] [Review][Patch][HIGH] PARTIAL — silent success. Real success/error indicator selectors are UNVERIFIED (need live session), so a hard success/error check is deferred. Mitigation applied: after the share click the code emits a `console.warn` that success is UNVERIFIED instead of asserting success silently. Full fix coupled to the live-verify defer. [api/services/facebookAutomation.js#shareSinglePost]
- [~] [Review][Patch][MEDIUM] PARTIAL — `alreadyShared`. Real detection needs the live-verified shared-state indicator (deferred). The merge is now annotated as forward-compatible (activates once `shareSinglePost` returns `alreadyShared`); kept rather than removed so the verify pass only adds detection, no re-plumbing. [api/services/facebookAutomation.js#shareFacebookPosts]
- [x] [Review][Patch][MEDIUM] FIXED — `evaluateHandle` over-match. Strict equality (`labels.includes(textContent.trim())`) and node set narrowed to `[role="menuitem"], [role="button"]` (dropped `div[role="none"] span`). [api/services/facebookAutomation.js#shareSinglePost]
- [x] [Review][Patch][MEDIUM] FIXED — duplicate-URL Map collision. `shareFacebookPosts` now rejects duplicate `postUrls` up front (`new Set(postUrls).size !== postUrls.length → throw`); test added. [api/services/facebookAutomation.js#shareFacebookPosts]
- [x] [Review][Patch][MEDIUM] FIXED — URL scheme/host validation (AC4.10 + SSRF). Each `postUrl` is parsed with `new URL()` (throws → clear error), required `http(s)` scheme + `facebook.com`/`*.facebook.com` host, all before navigation. Tests added (file:, internal host, malformed). [api/services/facebookAutomation.js#shareFacebookPosts]
- [x] [Review][Patch][MEDIUM] FIXED — swallowed catch. The "Share now" `waitForSelector` catch now re-throws non-timeout errors (frame destroyed / context destroyed) and only swallows selector-timeout. [api/services/facebookAutomation.js#shareSinglePost]
- [x] [Review][Patch][LOW] FIXED — misleading guard removed; `handle.asElement()` replaces the `if (shareNowEl)` + `.evaluate(n===null)` dance. [api/services/facebookAutomation.js#shareSinglePost]
