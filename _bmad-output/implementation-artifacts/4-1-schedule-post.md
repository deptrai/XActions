...
- [x] [Review][Patch][P0/HIGH] FIXED — moved `startFacebookScheduler()` from the `isWorkerEntry` block in `jobQueue.js` into `api/server.js`'s `listen` callback (after `global.io = io`), gated by `ENABLE_FB_SCHEDULER !== 'false'`. Removed the now-dead `pathToFileURL`/`isWorkerEntry`/import from `jobQueue.js`. `facebook:operation` emits now reach connected clients. [api/server.js:609; api/services/jobQueue.js]

- [x] [Review][Patch][HIGH] FIXED — duplicate execution on overlapping ticks. Added a `running` state; each row is now claimed atomically via `updateMany({ where:{ id, status:'pending' }, data:{ status:'running' } })` and skipped if `count===0`. Added `sweepStaleRunning()` on scheduler startup to recover `running` rows left by a crashed process → `failed`. Verified by the new "claims the row atomically" test (concurrent ticks → exactly 1 execution). [api/services/facebookScheduler.js]
- [x] [Review][Patch][HIGH] FIXED — silent post failure. The executor return value is now inspected via `isPostSuccess()` (handles the real `batchResult` `{succeeded,failed,results}` shape); on failure the worker throws → `status:failed`. Test executor rewritten to return the real batchResult shape; added "post that returns a failure result (no throw) is marked failed" test. [api/services/facebookScheduler.js; tests/services/facebook-schedule.test.js]
- [x] [Review][Patch][MEDIUM] FIXED — jitter-defer now bases on `now`: `new Date(now.getTime() + jitter)`, so an overdue capped post advances past the cap window instead of busy-looping. [api/services/facebookScheduler.js]
- [x] [Review][Patch][MEDIUM] FIXED — each loop iteration is wrapped in its own try/catch; a failure in claim/count/operation.create logs and continues, never aborting the remaining due rows. [api/services/facebookScheduler.js]
- [x] [Review][Patch][MEDIUM] FIXED — NFR3 test now routes the fake cookie through a throwing executor's error message and asserts it is scrubbed from every persisted field; confirms `status:failed` ran. Test verified passing. [tests/services/facebook-schedule.test.js]
- [x] [Review][Patch][MEDIUM] FIXED — `safeErrorString()` allowlists `err.code`/`err.name` only (and rejects bare `'Error'`); never persists raw `err.message`. NFR3 test proves a cookie in a message never lands in a persisted field. [api/services/facebookScheduler.js]
- [x] [Review][Patch][MEDIUM] FIXED — removed the symlink-fragile `isWorkerEntry` check entirely (superseded by P0: scheduler now starts in the server process, not via worker-entry detection). [api/services/jobQueue.js]
- [x] [Review][Patch][LOW] FIXED — `ENABLE_FB_SCHEDULER=true` documented in `.env.example` with a comment; schema `status` field annotated with the state machine. [.env.example; prisma/schema.prisma]
