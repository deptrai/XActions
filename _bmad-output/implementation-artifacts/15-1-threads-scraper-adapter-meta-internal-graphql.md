...
- [ ] [Review][Decision] THR-DEC-01: AbstractApiClient 5xx and transport-error retry/quarantine ownership — `AbstractApiClient` catches transport errors, wraps them as a 503 `ProxyDeadError` at `src/core/base-client.js:564-573`, and throws `ProxyDeadError` for `status >= 500` at `src/core/base-client.js:820-828` without retrying the same proxy or quarantining. Since `base-client.js` is NO TOUCH for 15.1 (core work from 11.3/13.1), decide whether to add 5xx/transport retry and quarantine logic in `AbstractApiClient` or implement a `ThreadsClient`-level wrapper/backoff for token and GraphQL calls. (edge)
- [ ] [Review][Decision] THR-DEC-02: CommentTreeExtractor cursor-deduplication and empty-cursor guard — `CommentTreeExtractor.fetchLayerPaginated` has no `seen` cursor set and exits when `end_cursor` is empty/null even if `has_next_page` is true (`src/scrapers/social/comment-tree.js:134-158`), so a repeated/stuck cursor can infinite-loop or stop prematurely. Since `comment-tree.js` is NO TOUCH for 15.1 (Story 14.1), decide whether to (a) request the fix in `comment-tree.js` or (b) implement a guard in `ThreadsCrawler.fetchLayer`. (edge)
- [ ] [Review][Decision] THR-DEC-03: get_post_comments BarcelonaPostPageQuery fallback for comment pagination — `fetchLayer` falls back to `POST_DETAIL` (`src/scrapers/social/threads/crawler.js:562`) for both root and reply layers. That query returns the whole `reply_threads` bundle in one call, so `maxDepth=0` still gets nested replies and every parent fetches the same full bundle, mis-parenting/duplicating comments. Decide whether to (a) remove the fallback and require captured `COMMENT_ROOTS`/`COMMENT_REPLIES` `doc_id`s (throwing `XACT_5000` if not configured), or (b) keep it short-term and clamp/flatten the returned tree. (edge)
- [ ] [Review][Decision] THR-DEC-04: Token cache and token-extraction failure need a proxy-aware contract — `ensureLsd` is keyed by `accountId`, not the resolved proxy, so a rotated proxy can reuse stale, IP-bound tokens (`src/scrapers/social/threads/client.js:93-110,208-211`). Token extraction failure throws `XACT_4010` but does not quarantine the proxy or trigger backoff. Decide whether `AbstractApiClient` should expose a proxy-aware token-caching hook/pipeline (cache + invalidate + quarantine on rotation), or `ThreadsClient` should manage a composite cache key and explicit invalidation. (edge)
